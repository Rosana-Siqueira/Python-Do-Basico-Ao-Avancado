# A partir do seguinte código aplique raise para apresentar possíveis erros do seguinte programa que realiza a escolha de uma das opções de lazer para uma pessoa e dependendo da escolha, chama uma função e realiza testes específicos.
...
def fazer_compras(nome,idade,valor):
    ...
    if type(valor) is not float:
        raise TypeError('Valor em dinheiro deve ser float')
    if valor < 200:
        print('Seu orçamento está curto, cuidado!')
...
if atividade != 'viajar_exterior' and atividade != 'fazer_compras' and atividade != 'cassino':
    raise NameError('Nomeclatura para função errada')
